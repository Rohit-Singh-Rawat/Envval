...
export function getVSCodeConfig(): VSCodeConfig {
	const config = workspace.getConfiguration(VSCODE_CONFIG_SECTION);
	return {
		pollIntervalSeconds: config.get<number>('pollIntervalSeconds', DEFAULT_POLL_INTERVAL_SECONDS),
		loggingVerbose: config.get<boolean>('loggingVerbose', DEFAULT_LOGGING_VERBOSE),
	};
}
...
export function onConfigChange(listener: (config: RuntimeConfig) => void): Disposable {
