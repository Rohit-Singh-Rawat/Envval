...
function detectSubmodule(workspacePath: string): {
  isSubmodule: boolean;
  parentPath?: string;
} {
  ...
  try {
    ...
    if (match) {
      ...
      return {
        isSubmodule: true,
        parentPath: path.dirname(path.dirname(gitdirPath)),
      };
    }
  } catch {
    // ignore read errors
...
function detectWorktree(workspacePath: string): {
  isWorktree: boolean;
  mainRepoPath?: string;
} {
  const gitFile = path.join(workspacePath, ".git");
  ...
  try {
    ...
    if (match) {
      ...
      if (path.basename(worktreesDir) === "worktrees") {
        return {
          isWorktree: true,
          mainRepoPath: path.dirname(path.dirname(worktreesDir)),
        };
      }
    }
  } catch {
...
function shouldIgnoreEnvFile(fileName: string): boolean {
  ...
  return IGNORED_ENV_FILES.some(
    (ignored) => baseName === ignored.toLowerCase(),
  );
}

async function getPackageJsonSignature(
  workspacePath: string,
  ...
  try {
    ...
    return pkg.repository?.url
      ? `${base}:${normalizeGitUrl(pkg.repository.url)}`
      : base;
  } catch {
    return undefined;
  }
...
async function getProjectContentSignature(
  workspacePath: string,
): Promise<string> {
...
export async function getWorkspacePath(): Promise<string | undefined> {
  return WorkspaceContextProvider.getInstance().getWorkspaceContext()
    .primaryPath;
}
 ...
 */
export async function getAllGitRemotes(
  workspacePath: string,
...
export async function getGitRemote(
  workspacePath: string,
): Promise<GitRemoteResult | undefined> {
  ...
  if (worktree.isWorktree && worktree.mainRepoPath) {
    ...
    if (main) {
      return {
        remoteUrl: main.remoteUrl,
        type: "worktree",
        parentRemote: main.remoteUrl,
      };
    }
  }
...
export async function getNormalizedGitRemote(
  workspacePath: string,
): Promise<string | undefined> {
...
export async function detectMonorepoStructure(
  ...
  try {
    ...
    const hasMarkers = monorepoMarkers.some((f) =>
...
export async function getCurrentWorkspaceId(
  ...
  if (gitRemoteResult) {
    ...
    logger?.debug(
      `Using Git remote: ${normalizedRemote} (${gitRemoteResult.type})`,
    );
  ...
  const storedGitRemote =
    identityStore?.getMostRecentGitRemote(workspacePath) ?? null;
  if (storedGitRemote) {
  ...
  return {
    ...
    availableSubProjects:
      availableSubProjects.length > 0 ? availableSubProjects : undefined,
    ...buildMigrationInfo(
      storedIdentity?.lastActiveRepoId,
      repoId,
  ...
  try {
    ...
    const relativePaths = uris
      ...
      .filter((rel) => {
        ...
        if (ignored) {
          logger?.debug(`Ignoring ${normalized} (ignore list)`);
        }
        if (!valid) {
          logger?.debug(`Ignoring ${normalized} (invalid filename)`);
        }
