...
export class ConnectionMonitor implements Disposable {
  ...
  public readonly onDidChangeConnectionState =
    this._onDidChangeConnectionState.event;

  private readonly _onDidChangeDetailedState =
    new EventEmitter<ConnectionState>();
  public readonly onDidChangeDetailedState =
    this._onDidChangeDetailedState.event;
  ...
  public static getInstance(logger?: Logger): ConnectionMonitor {
    if (!ConnectionMonitor.instance) {
      if (!logger) {
        throw new Error(
          "ConnectionMonitor must be initialized with a logger before use",
        );
      }
  ...
  private async checkHealth(): Promise<boolean> {
    try {
      ...
      if (
        ...
        message.includes("fetch is not a function")
      ) {
        this.logger.warn(
