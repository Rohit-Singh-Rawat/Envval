...
export class WorkspaceValidator {
	...
	public validateWorkspace(
		...
		this.validationInFlight = this.runValidation(workspacePath, showPrompts).finally(() => {
	...
	public resetValidation(): void {
		...
		this.logger.debug('Session validation reset — will re-validate on next call.');
	}

	// ─── Private ────────────────────────────────────────────────────────────────

	private async runValidation(workspacePath: string, showPrompts: boolean): Promise<ValidationResult> {
		...
		if (!safetyResult.isSafe) {
			...
			if (showPrompts) {
				canProceed = await this.promptForUnsafePath(workspacePath, safetyResult);
			}
	...
	private promptForUnsafePath(workspacePath: string, safetyResult: PathSafetyResult): Promise<boolean> {
		const isCritical =
			safetyResult.reason === 'root_drive' || safetyResult.reason === 'system_directory';
	...
	private async showCriticalBlockingError(
		...
		const reasonText = REASON_TEXT[safetyResult.reason ?? ''] ?? 'unsafe location';
