...
import { OPERATION_QUEUE_STORAGE_KEY, OPERATION_QUEUE_MAX_SIZE } from '../lib/constants';
...
export class OperationQueueService implements Disposable {
	...
	private constructor(ctx: ExtensionContext, logger: Logger) {
		this.ctx = ctx;
		this.logger = logger;
		this.queue = ctx.workspaceState.get<QueuedOperation[]>(OPERATION_QUEUE_STORAGE_KEY, []);
		this.logger.debug(`OperationQueueService: Loaded ${this.queue.length} queued operations`);
	}

	public static getInstance(ctx?: ExtensionContext, logger?: Logger): OperationQueueService {
		if (!OperationQueueService.instance) {
			if (!ctx || !logger) {
				throw new Error('OperationQueueService must be initialized with context and logger before use');
			}
	...
	public async enqueue(type: OperationType, envId: string, fileName: string): Promise<void> {
		// Deduplicate: remove existing entry with same (envId, type)
		this.queue = this.queue.filter((op) => !(op.envId === envId && op.type === type));
		...
		if (this.queue.length >= OPERATION_QUEUE_MAX_SIZE) {
			...
			for (let i = 0; i < this.queue.length; i++) {
				...
				if (
					...
					(op.priority === highestPriority && (oldestTimestamp === '' || op.queuedAt < oldestTimestamp))
				) {
			...
			if (evictIndex !== -1) {
				...
				this.logger.debug(`OperationQueueService: Evicted ${evicted.id} to make room`);
			}
		}
	...
	public async processAll(
		...
		this.logger.info(`OperationQueueService: Processing ${snapshot.length} operations`);
		...
		try {
			for (const op of snapshot) {
				try {
					...
					this.logger.error(`OperationQueueService: Failed to process ${op.id}: ${error}`);
					failed++;
				}
			}
		...
		this.logger.info(`OperationQueueService: Completed — ${succeeded} succeeded, ${failed} failed`);
	...
	private async persist(): Promise<void> {
		await this.ctx.workspaceState.update(OPERATION_QUEUE_STORAGE_KEY, this.queue);
	}
