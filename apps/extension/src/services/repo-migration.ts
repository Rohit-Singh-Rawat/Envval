...
export class RepoMigrationService {
  ...
  async detectMigrationNeeded(workspacePath: string): Promise<{
    ...
    try {
      ...
      this.logger.error(
        `Error detecting migration need: ${formatError(error)}`,
      );
      return { needsMigration: false };
    }
  }
  ...
  async performMigration(
    ...
    try {
      ...
      const migratedCount = await this.metadataStore.migrateRepoId(
        oldRepoId,
        newRepoId,
      );
      ...
      try {
        ...
        const result = await this.apiClient.migrateRepo(
          oldRepoId,
          newRepoId,
          gitRemoteUrl,
        );
      ...
      this.logger.info(
        `Successfully migrated ${migratedCount} metadata entries`,
      );
      ...
      vscode.window.showErrorMessage(
        "Repository identity migration failed. Please try again.",
      );
   ...
   */
  async handleAutomaticMigrationPrompt(
    workspacePath: string,
  ): Promise<boolean> {
