...
export class EnvvalMetadataStore {
	...
	public async saveAllMetadata(metadata: Map<string, EnvMetadata>): Promise<void> {
		return this.enqueueWrite(async () => {
			await this.ctx.workspaceState.update(METADATA_STORAGE_KEY, Object.fromEntries(metadata));
	...
	public async clearMetadata(envId?: string): Promise<void> {
		if (envId) {
			return this.enqueueWrite(async () => {
				const stored =
					this.ctx.workspaceState.get<EnvvalMetadataStorage>(METADATA_STORAGE_KEY) ?? {};
				delete stored[envId];
				await this.ctx.workspaceState.update(METADATA_STORAGE_KEY, stored);
				this._onDidChangeMetadata.fire(envId);
			});
	...
	public async getEnvsByRepoId(repoId: string): Promise<EnvMetadata[]> {
		const stored = this.ctx.workspaceState.get<EnvvalMetadataStorage>(METADATA_STORAGE_KEY) ?? {};
		return Object.values(stored).filter((m) => computeEnvId(repoId, m.fileName) === m.envId);
	}
}
