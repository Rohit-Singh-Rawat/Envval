  ...
  implements vscode.TreeDataProvider<EnvItem | FolderItem>, vscode.Disposable
{
  ...
  constructor(
    ...
    this.disposables.push(
      this.fileWatcher.onDidChange((event) => {
        this.logger.debug(
          `File changed: ${event.fileName}, refreshing tree view`,
        );
      ...
      this.fileWatcher.onDidCreate((event) => {
        this.logger.debug(
          `File created: ${event.fileName}, refreshing tree view`,
        );
        this.refresh();
      }),
  ...
  async getChildren(
    element?: EnvItem | FolderItem,
  ): Promise<(EnvItem | FolderItem)[]> {
    try {
...
export class EnvItem extends vscode.TreeItem {
  ...
  private createTooltip(
    ...
    md.appendMarkdown(
      `**Last Sync:** ${lastSynced.toLocaleString()} (${relativeTime})  \n`,
    );
    md.appendMarkdown(`\n---\n\n`);
