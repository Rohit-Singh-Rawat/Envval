...
const LANGUAGE_PATTERNS: Record<string, LanguagePattern[]> = {
	...
	csharp: [
		{
			pattern: /Environment\.GetEnvironmentVariable\(['"]([A-Za-z_][A-Za-z0-9_]*)['"]\)/g,
			extractKey: (m) => m[1],
		},
	],
	shellscript: [
...
const SENSITIVE_PATTERNS = ['KEY', 'SECRET', 'PASSWORD', 'TOKEN', 'CREDENTIAL', 'AUTH', 'PRIVATE'];
...
export class EnvHoverProvider implements HoverProvider {
	...
	public async provideHover(
		...
		const envKey = this.findEnvKeyAtPosition(line, position.character, languageId);
		if (!envKey) {
			return null;
		}
	...
	private findEnvKeyAtPosition(
		...
		while ((match = FALLBACK_PATTERN.pattern.exec(line)) !== null) {
			...
			if (character >= start && character <= end && this.envCache.hasVariable(key)) {
				return key;
			}
		}
