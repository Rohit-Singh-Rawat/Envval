...
export class AuthenticationProvider {
    ...
    try {
      while (totalWaitTime < expiresIn && this.isPolling) {
        ...
        try {
          ...
          await this.secretsManager.setWrappedKeyMaterial(
            data.wrapped_key_material,
          );
          ...
          if (!keyMaterial) {
            throw new Error(
              "Failed to decrypt key material with stored private key",
            );
          }
          ...
          switch (errorCode) {
            case "authorization_pending":
              ...
              if (pollingInterval !== previousInterval) {
                ...
                this.logger.debug(
                  `Polling attempt ${attempts}: authorization pending`,
                );
              }
              break;
            ...
            default:
              this.logger.error(
                `Polling error: ${errorCode} - ${errorDescription}`,
              );
  ...
  public async handleTokenRefreshFailure(): Promise<void> {
    ...
    vscode.window.showWarningMessage(
      "Envval: Session expired. Please log in again.",
    );
  }
