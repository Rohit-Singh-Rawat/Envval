...
class ApiClient {
  ...
  private async handleRetry(error: AxiosError): Promise<unknown> {
    ...
    if (!isRetryable) {
      this.logger?.error(
        `[API] ${method} ${url} failed: ${this.extractErrorMessage(error)}`,
      );
      return Promise.reject(error);
    }
  ...
  private checkCircuitBreaker(): void {
    ...
    if (this.circuitState === "open") {
      ...
      if (elapsed >= CIRCUIT_BREAKER_COOLDOWN_MS) {
        ...
        this.logger?.info(
          "[API] Circuit breaker HALF-OPEN - testing connection",
        );
        return;
      }
      throw new ApiError(
...
export class EnvvalApiClient extends ApiClient {
  ...
  private setupAuthInterceptors(): void {
    ...
    this.client.interceptors.response.use(
      ...
      async (error) => {
        ...
        if (!newToken) {
          this.logger?.error(
            "[API] Session could not be refreshed — logging out",
          );
          await this.authProvider.handleTokenRefreshFailure();
          return Promise.reject(error);
        }
  ...
  public async checkEnvExists(
    repoId: string,
    fileName: string,
  ): Promise<EnvExistsResponse> {
  ...
  public async getEnvMetadata(envId: string): Promise<EnvMetadata> {
    return this.get<EnvMetadata>(
      `${API_V1}/envs/${encodeURIComponent(envId)}/metadata`,
    );
  }
  ...
  public async updateEnv(envId: string, data: UpdateEnvData): Promise<Env> {
    return this.put<Env, UpdateEnvData>(
      `${API_V1}/envs/${encodeURIComponent(envId)}`,
      data,
    );
  }
