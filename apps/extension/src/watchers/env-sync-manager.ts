...
export class SyncManager {
      ...
      for (const localMeta of trackedEnvs) {
        try {
          ...
          if (localHash === localMeta.lastSyncedHash) {
            ...
            await this.metadataStore.saveEnvMetadataSync(localMeta.envId, localMeta.fileName, remoteHash);
  ...
  public async handleEnvFileSave(uri: Uri): Promise<void> {
    try {
      ...
      await this.metadataStore.saveEnvMetadataSync(envId, metadata.fileName, hash);
