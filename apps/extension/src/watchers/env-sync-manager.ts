...
export class SyncManager implements Disposable {
  ...
  private constructor(
    ...
    const subscriptions = [
      ...
      envFileWatcher.onDidDelete((event) =>
        this.handleDeletedEnvFile(event.uri),
      ),
    ];
        ...
        try {
          ...
          try {
            ...
            if (error instanceof ApiError && error.status === 404) {
              this.logger.warn(
                `Remote env ${localMeta.envId} missing — triggering reconciliation`,
              );
          ...
          if (!ciphertext || !iv) {
            this.logger.error(
              `Invalid remote format for ${localMeta.fileName}`,
            );
            continue;
          }
          ...
          const freshMeta = await this.metadataStore.loadEnvMetadata(
            localMeta.envId,
          );
          if (!freshMeta) {
            this.logger.debug(
              `[SyncManager] Metadata cleared for ${localMeta.fileName} during poll — skipping`,
            );
            continue;
          }
          ...
          this.logger.error(
            `Error polling ${localMeta.fileName}: ${formatError(error)}`,
          );
        } finally {
  ...
  private async applyRemoteUpdate(
    ...
    if (!targetEnvId) {
      ...
      targetEnvId =
        existing?.envId ??
        (await getRepoAndEnvIds(fileName, this.context))?.envId;
    }
    if (targetEnvId) {
  ...
  public async handleEnvFileSave(uri: Uri): Promise<void> {
    ...
    try {
      ...
      this.logger.error(
        `Auto-sync failed for ${fileName}: ${formatError(error)}`,
      );
    } finally {
  ...
  private async handle404OnSave(
    ...
    if (failures >= SyncManager.MAX_CONSECUTIVE_404S) {
      this.logger.error(
        `${failures} consecutive 404s for ${fileName} — halting auto-recovery`,
      );
      window.showErrorMessage(
    ...
    this.logger.debug(
      `404 attempt ${failures}/${SyncManager.MAX_CONSECUTIVE_404S} for ${fileName}`,
    );
    await this.metadataStore.clearMetadata(envId);
    await this.envInitService.maybeInitializeOrRestore(uri);
  }

  private async handle412OnSave(
    envId: string,
    fileName: string,
    uri: Uri,
  ): Promise<void> {
  ...
  public async pushEnv(envId: string): Promise<void> {
    ...
    if (!ConnectionMonitor.getInstance().isOnline) {
      OperationQueueService.getInstance().enqueue(
        "push",
        envId,
        metadata.fileName,
      );
      window.showInformationMessage(
        `Envval: Queued push for ${metadata.fileName} (offline)`,
      );
      return;
    }
