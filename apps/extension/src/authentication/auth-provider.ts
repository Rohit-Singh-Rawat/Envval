...
export class AuthenticationProvider {
  ...
  // Singleton pattern
  public static getInstance(secretsManager: EnvVaultVsCodeSecrets, logger?: Logger): AuthenticationProvider {
  ...
  public async initiateLogin(): Promise<void> {
    try {
      ...
      vscode.window.showErrorMessage('EnvVault: Failed to initiate login. Please try again.');
    }
  }

  public async handleAuthenticationCallback(callbackUri: vscode.Uri): Promise<void> {
    try {
      this.logger.info(`Handling EnvVault auth callback: ${callbackUri.toString()}`);
      ...
      if (!refreshToken) {
        ...
        vscode.window.showErrorMessage('EnvVault: Authentication failed - no token received.');
        return;
      }
      ...
      vscode.window.showInformationMessage('EnvVault: Successfully logged in!');
    } catch (error) {
      this.logger.error(`Failed to handle EnvVault auth callback: ${(error as Error).message}`);
      vscode.window.showErrorMessage('EnvVault: Authentication failed. Please try again.');
    }
  }
