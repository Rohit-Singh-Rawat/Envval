	...
	public registerHandlers(
		...
		context.subscriptions.push(
			...
			vscode.commands.registerCommand(Commands.SHOW_QUICK_SYNC_ACTION, () =>
				handleQuickSyncAction(context.extensionMode === vscode.ExtensionMode.Development)
			),
			...
			vscode.commands.registerCommand(
				...
				async (itemOrEnvId?: string | { metadata: { envId: string } }) => {
					...
					if (envId) {
						...
						vscode.window.showErrorMessage('Envval: No environment specified for sync.');
					}
				}
			),
		...
		if (context.extensionMode === vscode.ExtensionMode.Development) {
			context.subscriptions.push(
				vscode.commands.registerCommand(Commands.DEV_CLEAR_ALL_STATE, async () => {
					await Promise.all([
						context.globalState.update(REPO_IDENTITIES_STORAGE_KEY, undefined),
						context.workspaceState.update(METADATA_STORAGE_KEY, undefined),
						context.workspaceState.update(OPERATION_QUEUE_STORAGE_KEY, undefined),
						EnvvalVsCodeSecrets.getInstance().clearAll(),
					]);
					vscode.window.showInformationMessage('[Dev] All state cleared. Reloading...');
					vscode.commands.executeCommand('workbench.action.reloadWindow');
				})
			);
		}
