...
function getActions(isDevMode: boolean): ActionItem[] {
  const items: ActionItem[] = [
    {
      label: "$(sync) Force Sync",
      description: "Force sync your environment variables",
      commandId: Commands.FORCE_SYNC,
    },
    {
      label: "$(plug) Retry Connection",
      description: "Test server connectivity",
      commandId: Commands.RETRY_CONNECTION,
    },
    {
      label: "$(output) Show Logs",
      description: "Open the extension output log",
      commandId: Commands.SHOW_LOGS,
    },
    {
      label: "$(sign-out) Logout",
      description: "Clear credentials and sign out",
      commandId: Commands.LOGOUT,
    },
  ];

  if (isDevMode) {
    items.push({
      label: "$(trash) [Dev] Clear All State",
      description: "Wipe all persisted state and reload",
      commandId: Commands.DEV_CLEAR_ALL_STATE,
    });
  }
