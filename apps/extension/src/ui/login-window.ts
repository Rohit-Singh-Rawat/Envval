...
export class LoginWindow {
	...
	public async show(): Promise<void> {
		...
		this.pollingStatusListener = this.authProvider.onPollingStatusChanged((status) =>
			this.handlePollingStatus(status)
		);
	...
	private async handleInitiateLogin(): Promise<void> {
		try {
			...
			const formattedCode = this.formatUserCode(this.currentDeviceCode.user_code);
			...
			this.sendMessage({ command: 'setState', state: 'error', message: errorMessage });
		}
	}
	...
	private async handleOpenBrowser(): Promise<void> {
		...
		const url =
			this.currentDeviceCode.verification_uri_complete || this.currentDeviceCode.verification_uri;
		try {
	...
	private handlePollingStatus(status: PollingStatus): void {
		switch (status.state) {
			...
			case 'expired':
				...
				break;
			case 'error':
				this.sendMessage({ command: 'setState', state: 'error', message: status.message });
				break;
			case 'polling':
