...
const itemVariants = (reduce: boolean | null) => ({
	...
	show: { opacity: 1, y: 0, transition: { duration: reduce ? 0.01 : 0.22, ease: EASE_OUT } },
});
...
function ResultSuccessScene() {
	...
	return (
		...
		>
			...
			<motion.h2 variants={itemVariants(reduce)} className="mt-6 text-xl font-semibold tracking-tight text-foreground">
				Device approved
			</motion.h2>
			<motion.p variants={itemVariants(reduce)} className="mt-2 text-sm text-muted-foreground">
