...
import { EASE_OUT, ILLUSTRATION_HEIGHT, getPos, ActivityIcon, AnimatedCursor, EnvVaultIcon } from './shared';
 ...
 */
export function StepVSCodeInner({ step, onComplete }: { step: VSCodeStep; onComplete?: () => void }) {
	...
	const runAnimation = useCallback(async () => {
		...
		const pos = (sel: string) => getPos(scope.current!, scope.current!.querySelector(sel) as HTMLElement);
	...
	return (
		...
		>
			...
			<div
				...
				style={{ background: 'radial-gradient(circle at 65% 40%, rgba(34, 197, 94, 0.15), transparent 50%)', opacity: 0 }}
			/>
...
/** Step 1 animation — searches marketplace, clicks EnvVault, installs extension. */
async function runInstallAnimation(animate: AnimateFn, pos: PosFn, cancelled: CancelledFn) {
	const search = pos('.search-input-container');
	const envVaultItem = pos('.envvault-list-item');
	...
	await animate('.search-char', { opacity: 1 }, { duration: 0.08, delay: (i: number) => i * 0.06, ease: EASE_OUT });
	await new Promise((r) => setTimeout(r, 300));
	if (cancelled()) return;

	await animate('.extension-item', { opacity: 1, x: 0 }, { duration: 0.35, ease: EASE_OUT, delay: (i: number) => i * 0.06 });
	await new Promise((r) => setTimeout(r, 400));
	if (cancelled()) return;

	await animate('.cursor', { x: envVaultItem.x, y: envVaultItem.y }, { duration: 0.45, ease: EASE_OUT });
	...
	await Promise.all([
		animate('.detail-panel', { opacity: 1 }, { duration: 0.35, ease: EASE_OUT }),
		animate('.detail-panel-placeholder', { opacity: 0 }, { duration: 0.3, ease: EASE_OUT }),
	]);
	await new Promise((r) => setTimeout(r, 300));
	if (cancelled()) return;

	await animate('.cursor', { x: installBtn.x, y: installBtn.y }, { duration: 0.5, ease: EASE_OUT });
	...
	await animate('.cursor', { x: away.x, y: away.y, opacity: 0 }, { duration: 0.45, ease: EASE_OUT });
...
async function runSecureAnimation(animate: AnimateFn, pos: PosFn, cancelled: CancelledFn) {
	...
	await animate('.cursor', { x: editorArea.x, y: editorArea.y }, { duration: 0 });
	...
	await animate('.command-palette', { opacity: 1, y: 0, scale: 1 }, { duration: 0.2, ease: EASE_OUT });
	await new Promise((r) => setTimeout(r, 200));
	if (cancelled()) return;

	await animate('.palette-char', { opacity: 1 }, { duration: 0.06, delay: (i: number) => i * 0.04, ease: EASE_OUT });
	await new Promise((r) => setTimeout(r, 200));
	if (cancelled()) return;

	await animate('.palette-item', { opacity: 1, x: 0 }, { duration: 0.25, ease: EASE_OUT, delay: (i: number) => i * 0.04 });
	await new Promise((r) => setTimeout(r, 200));
	if (cancelled()) return;

	await animate('.cursor', { x: pushItem.x, y: pushItem.y }, { duration: 0.4, ease: EASE_OUT });
	...
	await animate('.command-palette', { opacity: 0, y: -4, scale: 0.98 }, { duration: 0.15, ease: EASE_OUT });

	await animate('.cursor', { x: away.x, y: away.y, opacity: 0 }, { duration: 0.4, ease: EASE_OUT });
	if (cancelled()) return;

	await animate('.encrypting-indicator', { opacity: 1, y: 0 }, { duration: 0.2, ease: EASE_OUT });
	...
	for (let i = 0; i < ENV_LINES.length; i++) {
		...
		await Promise.all([
			...
			animate(`.env-enc-${i}`, { opacity: 1 }, { duration: 0.2, ease: EASE_OUT }),
			animate(`.lock-check-${i}`, { opacity: 1, scale: 1 }, { duration: 0.2, ease: EASE_OUT }),
		]);
	}

	await animate('.encrypting-indicator', { opacity: 0, y: -4 }, { duration: 0.15 });

	await animate('.toast-notification', { opacity: 1, y: 0, x: 0 }, { duration: 0.3, ease: EASE_OUT });
	...
	return (
		<>
			...
			<div className='w-[100px] border-r border-border bg-muted/20 flex flex-col'>
				<div className='search-input-container border-b border-border p-1'>
					<div className='flex items-center gap-1.5 rounded-sm border border-border bg-background px-2 py-[3px]'>
						<Search01Icon size={10} className='text-muted-foreground/50 shrink-0' strokeWidth={2} />
						<span className='text-[10px] text-foreground/80 truncate flex min-w-0'>
							{['e', 'n', 'v', 'v', 'a', 'u', 'l', 't'].map((char, i) => (
								<span key={i} className='search-char' style={{ opacity: 0 }}>{char}</span>
							))}
						</span>
					</div>
				</div>
				<div className='px-2 h-fit py-0'>
					<span className='text-[8px] font-semibold uppercase tracking text-muted-foreground/60'>Marketplace</span>
				</div>
				<div className='px-1 space-y-0.5 flex-1 overflow-hidden'>
					<ExtensionListItem name='EnvVault' publisher='EnvVault' active className='envvault-list-item' />
			...
			<div className='flex-1 relative bg-background overflow-hidden'>
				...
				<div className='detail-panel flex-1 bg-background overflow-hidden flex flex-col relative' style={{ opacity: 0 }}>
					<div className='flex items-start gap-2 border-b border-border p-2 py-3'>
						...
						<div className='flex-1 min-w-0'>
							<div className='flex items-center gap-1.5'>
								...
								<span className='rounded bg-muted px-1 py-px text-[8px] font-medium text-muted-foreground'>v2.4.1</span>
							</div>
							<p className='mt-px text-[9px] text-muted-foreground'>Secure .env management & team sharing</p>
							...
							<div className='mt-1.5 flex items-center gap-1'>
								<button className='install-btn relative flex h-4.5 w-14 items-center justify-center rounded-sm text-[9px] font-medium text-primary-foreground bg-primary'>
									<span className='btn-text-install absolute flex items-center gap-0.5'>
										<Download01Icon size={8} strokeWidth={2.5} className='shrink-0' />
										Install
									</span>
									<span className='btn-text-installing absolute flex items-center gap-0.5' style={{ opacity: 0 }}>
										<motion.div
									...
									<span className='btn-text-installed absolute flex items-center gap-0.5' style={{ opacity: 0 }}>
										<Tick01Icon size={6} className='shrink-0' strokeWidth={2.5} />
										<span className='text-[8px]'>Installed</span>
									</span>
								</button>
								<button className='flex h-4.5 items-center justify-center rounded-sm border border-border bg-background px-1.5 text-[8px] text-muted-foreground'>Disable</button>
								<button className='flex h-4.5 items-center justify-center rounded-sm border border-border bg-background px-1.5 text-[8px] text-muted-foreground'>Uninstall</button>
							</div>
						</div>
					</div>
					<div className='flex border-b border-border px-2'>
						<div className='border-b-2 border-primary px-2 py-1 text-[9px] font-medium text-foreground'>Details</div>
						<div className='px-2 py-1 text-[9px] text-muted-foreground/60'>Feature Contributions</div>
					...
					<div className='p-2 flex-1 overflow-hidden'>
						<div className='space-y-1.5 text-[9px] text-muted-foreground'>
							{['End-to-end encrypted .env sync', 'Team sharing & granular access control', 'Auto-detect .env files in workspace'].map((f) => (
								<p key={f} className='flex items-center gap-2'>
									<span className='flex size-3.5 items-center justify-center rounded-sm bg-emerald-500/10'>
										<Tick01Icon size={6} className='text-emerald-500' strokeWidth={2.5} />
									</span>
									{f}
								</p>
							))}
		...
		<>
			...
			<div className='w-[100px] border-r border-border bg-muted/20 flex flex-col'>
				<div className='border-b border-border px-2 py-1'>
					<span className='text-[8px] font-semibold uppercase tracking text-muted-foreground/60'>Explorer</span>
				</div>
			...
			<div className='editor-area flex-1 bg-background overflow-hidden flex flex-col relative'>
						...
						<div className='flex-1 space-y-0'>
							{ENV_LINES.map((line, i) => (
								<div key={line.key} className='flex items-center'>
									<span className='text-emerald-600 dark:text-emerald-400 shrink-0'>{line.key}</span>
									<span className='text-muted-foreground/60 shrink-0'>=</span>
									<div className='relative min-w-0 flex-1'>
										<span className={`env-val-plain env-plain-${i} text-amber-600 dark:text-amber-400 block truncate`}>{line.value}</span>
										<span className={`env-val-enc env-enc-${i} text-muted-foreground/50 absolute inset-0 truncate`} style={{ opacity: 0 }}>{line.encrypted}</span>
									</div>
									<span className={`lock-check lock-check-${i} ml-1 shrink-0`} style={{ opacity: 0 }}>
										<Tick01Icon size={8} className='text-emerald-500' strokeWidth={2.5} />
									</span>
								</div>
							))}
						</div>
				...
				<div className='command-palette absolute top-0 left-1/2 -translate-x-1/2 w-[75%] mt-1 rounded-md border border-border bg-card shadow-xl overflow-hidden z-10' style={{ opacity: 0 }}>
					<div className='flex items-center gap-1.5 border-b border-border px-2 py-1.5'>
						<CommandIcon size={10} className='text-muted-foreground/50 shrink-0' strokeWidth={2} />
						<span className='text-[9px] text-foreground/80 flex'>
							{['>', ' ', 'E', 'n', 'v', 'V', 'a', 'u', 'l', 't', ':', ' ', 'P', 'u', 's', 'h'].map((char, i) => (
					...
					<div className='py-0.5'>
						{PALETTE_ITEMS.map((item, i) => (
							<div
								key={item}
								className={cn('palette-item flex items-center gap-2 px-2 py-1 text-[9px] relative', i === 0 ? 'palette-push-item' : '')}
								style={{ opacity: 0 }}
							>
								{i === 0 && <div className='palette-highlight absolute inset-x-0.5 inset-y-0 rounded-sm bg-accent' style={{ opacity: 0 }} />}
								<span className={cn('relative z-10', i === 0 ? 'text-foreground' : 'text-muted-foreground')}>{item}</span>
							</div>
						))}
					</div>
				</div>
				...
				<div className='encrypting-indicator absolute bottom-2 right-2 flex items-center gap-1.5 rounded-md bg-card border border-border px-2.5 py-1.5 shadow-sm' style={{ opacity: 0 }}>
					<motion.div
				...
				{/* Toast Notification */}
				<div className='toast-notification absolute bottom-2 right-2 flex items-center gap-1.5 rounded-md bg-card border border-border px-2 py-1.5 shadow-lg z-10' style={{ opacity: 0 }}>
...
function ExtensionListItem({ name, publisher, active, className }: { name: string; publisher: string; active?: boolean; className?: string }) {
	const isEnvVault = name === 'EnvVault';
	return (
		<div className={cn('extension-item flex items-center gap-1 rounded-sm p-1 cursor-pointer transition-colors', active ? 'bg-accent' : 'hover:bg-accent/50', className)} style={{ opacity: 0 }}>
			{isEnvVault ? (
				<div className='flex size-5 shrink-0 items-center justify-center rounded-[3px]'>
					<EnvVaultIcon size={10} />
				</div>
			) : (
				<div className='flex size-5 shrink-0 items-center justify-center rounded-[3px] bg-muted'>
					<span className='text-[7px] font-bold text-muted-foreground'>{name === '.ENV' ? '.E' : name[0]}</span>
				</div>
			)}
			<div className='min-w-0 flex-1'>
				<div className={cn('truncate text-[8px] font-medium leading-tight', active ? 'text-foreground' : 'text-muted-foreground')}>{name}</div>
				<div className='truncate text-[7px] text-muted-foreground/60 leading-tight'>{publisher}</div>
			</div>
		</div>
	);
}

function FileTreeItem({ name, isFolder, level = 0, active, expanded }: { name: string; isFolder?: boolean; level?: number; active?: boolean; expanded?: boolean }) {
	return (
		<div
			className={cn('flex items-center gap-1 rounded-sm px-1 py-px text-[8px] cursor-pointer transition-colors', active ? 'bg-accent text-foreground' : 'text-muted-foreground hover:bg-accent/50')}
			style={{ paddingLeft: `${4 + level * 8}px` }}
		>
			{isFolder && (
				<svg width='6' height='6' viewBox='0 0 6 6' fill='none' className={cn('shrink-0', expanded ? 'rotate-90' : '')} aria-hidden='true'>
					<path d='M2 1L4.5 3L2 5' stroke='currentColor' strokeWidth='1' strokeLinecap='round' strokeLinejoin='round' />
				</svg>
			)}
...
function Stars({ count, size = 8 }: { count: number; size?: number }) {
	return (
		<div className='flex items-center gap-px'>
			{[1, 2, 3, 4, 5].map((i) => (
				<StarIcon key={i} size={size} className={i <= count ? 'text-amber-400 fill-amber-400' : 'text-muted fill-muted'} />
			))}
		</div>
	);
}
...
function VSCodeLogo({ maskId }: { maskId: string }) {
	return (
		<svg xmlns='http://www.w3.org/2000/svg' width='48' height='48' viewBox='0 0 100 100' fill='none' className='opacity-25' aria-hidden='true'>
