 ...
 */
export function StepDashboardInner({
  onComplete,
}: {
  onComplete?: () => void;
}) {
  ...
  const runAnimation = useCallback(async () => {
    ...
    const pos = (sel: string) => {
      ...
      return el ? getPos(container, el) : { x: 0, y: 0 };
    };
  ...
  return (
    <div ref={scope} className="relative">
      ...
      <div className="flex" style={{ height: ILLUSTRATION_HEIGHT - 20 }}>
        {/* App Sidebar */}
            ...
            >
              ...
              <div className="flex items-center gap-2 text-[7px] text-muted-foreground/50">
                <span className="flex items-center gap-0.5">
                  <Key01Icon size={7} strokeWidth={2} />4 vars
                </span>
                <span className="flex items-center gap-0.5">
...
const SIDEBAR_ICON_MAP: Record<string, (cls: string) => React.ReactNode> = {
  dashboard: (cls) => (
    <DashboardSquare01Icon size={10} className={cls} strokeWidth={1.5} />
  ),
  integrations: (cls) => (
    <Link01Icon size={10} className={cls} strokeWidth={1.5} />
  ),
  devices: (cls) => (
    <DeviceAccessIcon size={10} className={cls} strokeWidth={1.5} />
  ),
  settings: (cls) => (
    <Settings02Icon size={10} className={cls} strokeWidth={1.5} />
  ),
};
