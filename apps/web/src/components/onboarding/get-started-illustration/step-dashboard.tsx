...
import { EASE_OUT, ILLUSTRATION_HEIGHT, getPos, AnimatedCursor, EnvVaultIcon } from './shared';
...
/**
 * Inner content for Step 3 — EnvVault dashboard with decryption.
...
export function StepDashboardInner({ onComplete }: { onComplete?: () => void }) {
	...
	const runAnimation = useCallback(async () => {
		...
		await animate('.sidebar-item', { opacity: 1, x: 0 }, { duration: 0.3, ease: EASE_OUT, delay: (i: number) => i * 0.05 });
		...
		await animate('.env-var-row', { opacity: 1, y: 0 }, { duration: 0.3, ease: EASE_OUT, delay: (i: number) => i * 0.06 });
		...
		await animate('.decrypt-badge', { opacity: 1, scale: 1 }, { type: 'spring', stiffness: 400, damping: 20 });
		...
		>
			...
			<div className='flex items-center justify-between border-b border-border bg-background px-3 py-1'>
				...
				<div className='flex items-center gap-2'>
					<div className='decrypt-badge flex items-center gap-1 rounded-full bg-emerald-500/10 border border-emerald-500/20 px-1.5 py-0.5' style={{ opacity: 0 }}>
						<motion.div
			...
			>
				...
				<div className='flex-1 bg-muted/20 overflow-hidden flex flex-col px-3 py-2'>
					<div className='repo-heading flex items-center gap-2 mb-2' style={{ opacity: 0 }}>
					...
					<div className='flex-1 overflow-hidden rounded-lg bg-muted/50 p-1'>
						<div className='env-card-header flex items-center justify-between px-2 py-1' style={{ opacity: 0 }}>
							<div className='flex items-center gap-1.5'>
								<span className='text-[9px] font-medium text-foreground'>.env.local</span>
								<span className='text-[7px] font-mono bg-background/50 px-1 py-px rounded border border-border/50 text-muted-foreground/60'>a7f2e3b1</span>
							</div>
							...
							{ENV_VARS.map((env, i) => (
								...
								>
									...
									<div className='flex-1 relative overflow-hidden min-w-0'>
										<span className={`env-masked env-masked-${i} text-muted-foreground/30 absolute truncate`}>{env.masked}</span>
										<span className={`env-revealed env-revealed-${i} text-muted-foreground truncate block`} style={{ opacity: 0 }}>{env.value}</span>
									</div>
									<div className='flex items-center gap-1 ml-1.5 shrink-0'>
										<span className={`reveal-check reveal-check-${i}`} style={{ opacity: 0 }}>
											<Tick01Icon size={7} className='text-emerald-500' strokeWidth={2.5} />
										</span>
										<div className='eye-toggle-0 relative size-3 flex items-center justify-center text-muted-foreground/40 cursor-pointer'>
											{i === 0 ? (
												<>
													...
													<span className='eye-icon-on absolute inset-0 flex items-center justify-center' style={{ opacity: 0 }}>
			...
			<div
				...
				style={{ background: 'radial-gradient(circle at 65% 50%, rgba(34, 197, 94, 0.12), transparent 50%)', opacity: 0 }}
			/>
...
const SIDEBAR_ICON_MAP: Record<string, (cls: string) => React.ReactNode> = {
	dashboard: (cls) => <DashboardSquare01Icon size={10} className={cls} strokeWidth={1.5} />,
	integrations: (cls) => <Link01Icon size={10} className={cls} strokeWidth={1.5} />,
	devices: (cls) => <DeviceAccessIcon size={10} className={cls} strokeWidth={1.5} />,
	settings: (cls) => <Settings02Icon size={10} className={cls} strokeWidth={1.5} />,
};
