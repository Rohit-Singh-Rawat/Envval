...
export function AppSidebar({ className }: AppSidebarProps) {
	...
	return (
		<Sidebar
			...
			variant="sidebar"
		>
			<SidebarHeader className="p-2 gap-5">
				<Link to="/" className="flex items-center gap-2 md:hidden">
					<EnvvalLogo variant="full" className="max-md:h-8 max-md:w-auto" />
				</Link>
					...
					<SidebarGroupContent>
						<SidebarMenu>
							{DASHBOARD_NAV_ITEMS.map((item) => {
								// Highlight for exact match AND nested routes (e.g. /devices/123)
								const isActive =
