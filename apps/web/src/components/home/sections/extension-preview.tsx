...
function SyncStatusBadge({
	...
	return (
		<div className="flex items-center gap-1.5">
			{status === "synced" && (
				<>
					<HugeiconsIcon icon={CloudIcon} className={cn("size-3", color)} />
					<span>Synced</span>
				</>
			)}
			{status === "modified" && (
	...
	return (
		...
		>
			{Array.from({ length: count }, (_, i) => (
				// biome-ignore lint/suspicious/noArrayIndexKey: pure presentational list
				<span
					key={i}
		...
		<div className="flex p-3">
			...
			<div className="flex-1 min-w-0">
				{INDEX_TS_LINES.map((line, i) => (
					...
					<div key={i} className="h-[22px] flex items-center relative">
						{line.tokens.map((token, j) => (
							// biome-ignore lint/suspicious/noArrayIndexKey: static code tokens
							<span
								key={j}
								className={cn(
...
function SidebarEnvCard({
	...
	return (
		// biome-ignore lint/a11y/useSemanticElements: custom styled sidebar button
		// biome-ignore lint/a11y/noStaticElementInteractions: custom interactive element
		<div
			onClick={onOpen}
		...
		>
			<div className="flex items-center gap-1.5 mb-1">
				<HugeiconsIcon icon={File01Icon} className={cn("size-3", info.color)} />
			...
			<div className="flex items-center gap-1.5 text-[9px] text-muted-foreground">
				...
				<div className="ml-auto">
					<SyncStatusBadge status={syncStatus} color={info.color} />
				</div>
			</div>
		</div>
	);
...
function ExtensionPreview({
	...
	useEffect(() => {
		...
		// biome-ignore lint/correctness/useExhaustiveDependencies: intro sequence should run independently
	}, [isInView]);
	...
	const renderEditorContent = () => {
		...
		return (
			<EnvEditor
				...
				onUpdateLine={(lineId, raw) =>
					actions.updateEnvLine(envKey, lineId, raw)
				}
		...
		>
			...
			<div className="flex flex-1 min-h-0">
				...
				<div className="w-64 bg-muted/70 flex-col border-r border-border hidden md:flex shrink-0">
					<div className="h-9 px-4 flex items-center justify-between text-[11px] font-medium tracking-wider text-muted-foreground">
						...
						<div className="flex gap-1">
							...
							/>
							<IconButton icon={MoreHorizontalIcon} label="More options" />
						</div>
					</div>
				...
				<div className="flex-1 flex flex-col min-w-0 bg-background relative overflow-hidden">
					...
					{state.openTabs.length !== 0 && (
						...
						>
							{state.openTabs.map((key) => {
								...
								const iconColor =
									key === "ts" ? "text-blue-500" : "text-muted-foreground";
								...
								>
									...
									<div className="flex items-center gap-2 text-xs text-muted-foreground px-4 py-2 sticky top-0 bg-background z-20">
										...
										<span className="text-foreground">
											{getFileInfo(state.activeTab).name}
										</span>
									</div>
									{renderEditorContent()}
								</motion.div>
					...
					<AnimatePresence>
						{state.showDownloadCTA && <DownloadCTA />}
					</AnimatePresence>
				</div>
			</div>
		</motion.div>
