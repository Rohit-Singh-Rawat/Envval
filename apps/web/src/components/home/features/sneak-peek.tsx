...
// ease-out per Emil: custom cubic-bezier over built-in CSS
const EASE_OUT = [0.32, 0.72, 0, 1] as const;
// ease-in-out for on-screen cursor movement (Emil: ease-in-out-onscreen)
...
async function runPeekSequence(animate: AnimateFn, cancelled: CancelledFn) {
	...
	// Fade cursor in gently (timing-300ms-max)
	...
	while (!cancelled()) {
		for (let i = 0; i < ENV_LINES.length; i++) {
			...
			// Move cursor — ease-in-out for on-screen movement, 500ms for natural feel
			await animate(
				'.cursor',
				{ x: `var(--env-x-${i})`, y: `var(--env-y-${i})` },
				{ duration: 0.5, ease: EASE_IN_OUT }
			);
			if (cancelled()) return;

			// Small settle pause before click
			await sleep(150);
			if (cancelled()) return;

			// Click — asymmetric timing (fast press, slower release per Emil)
			...
			// Tooltip fade in — gentle opacity + subtle scale + blur bridge
			await animate(
				tooltip,
			...
			// Tooltip fade out — soft dissolve, not snappy
			await Promise.all([
				animate(
					tooltip,
					{ opacity: 0, y: 4, scale: 0.98, filter: 'blur(4px)' },
...
const SneakPeekIllustration = () => {
	...
	return (
		<div className='flex flex-col flex-1 h-full border-r border-b border-muted rounded-br-2xl overflow-hidden'>
			<div
				ref={scope}
				className='relative ml-6 rounded-tl-lg border-l border-t overflow-hidden bg-card shadow-[inset_1px_1px_0_0_rgba(255,255,255,0.1),-4px_-4px_12px_-4px_rgba(255,255,255,0.08)] flex-1 flex flex-col'
				role='img'
			...
			>
				...
				<div className='p-3 pb-4 bg-background relative flex-1'>
					<div className='flex gap-3'>
						<div
							className='flex flex-col text-muted-foreground/20 select-none text-right shrink-0'
							style={{ width: 18 }}
						>
							{CODE_LINES.map((line) => (
								<span
									key={line.line}
									className='text-[11px] leading-[22px] font-mono text-muted-foreground/50'
								>
									{line.line}
								</span>
							))}
								...
								>
									{line.tokens.map((token, i) => (
										<span
											key={i}
											className={cn(
												'text-[11px] font-mono whitespace-pre leading-[22px]',
												token.cls
											)}
										>
											{token.text}
		...
		>
			...
			>
				<div className='flex items-center gap-1.5 px-2.5 py-1 border-b border-border bg-muted/40 h-3 box-content'>
					<span className='text-[9px] text-muted-foreground/50 font-medium leading-[18px]'>
						{peekFile}
					</span>
				</div>
