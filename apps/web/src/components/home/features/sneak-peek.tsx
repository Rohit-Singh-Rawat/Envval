...
import { EASE_IN_OUT, EASE_OUT } from "@/lib/animation";
...
const SneakPeekIllustration = () => {
  ...
  const startAnimation = useCallback(async () => {
    ...
    const envTargets = scopeEl.querySelectorAll(
      "[data-env-target]",
              ...
              {CODE_LINES.map((line) => (
                ...
                >
                  {line.tokens.map((token, i) => (
                    // biome-ignore lint/suspicious/noArrayIndexKey: tokens in a static code listing are positional
                    <span
                      key={i}
                      className={cn(
...
function EnvVariable({
  ...
  return (
    <span className="relative inline-flex items-center" data-env-target={index}>
      <span
        className={`env-target-${index} text-[11px] font-mono text-emerald-500 leading-[22px]`}
      >
        {envKey}
      </span>
      <span
      ...
      >
        ...
        <div className="px-2.5 py-1.5 flex items-baseline gap-0.5">
          <span className="text-[10px] font-mono text-foreground/70 font-medium">
            {peekValue}
          </span>
        </div>
      </div>
