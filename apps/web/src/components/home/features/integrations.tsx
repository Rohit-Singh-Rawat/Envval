...
import { EASE_OUT, EASE_IN_OUT, EASE_OUT_SMOOTH } from '@/lib/animation';
...
const IconVercel = (
	<svg x='0' y='0' width='24' height='24' viewBox='0 0 256 222'>
...
const IconCloudflare = (
	<svg x='0' y='0' width='24' height='24' viewBox='0 0 128 128'>
		<path fill='#fff' d='m115.679 69.288l-15.591-8.94l-2.689-1.163l-63.781.436v32.381h82.061z' />
...
const IconSlack = (
	<svg x='0' y='0' width='24' height='24' viewBox='0 0 256 256'>
...
const IconDatabase = (
	<g fill='none'>
		<path fill='#b2b2b2' d='M2.047 5.19v13.62C2.047 21.124 6.504 23 12 23s9.953-1.875 9.953-4.19V5.19z' />
		<path fill='#e3e3e3' d='M12 5.19H2.047v13.62C2.047 21.124 6.504 23 12 23z' />
		<path stroke='currentColor' strokeLinecap='round' strokeLinejoin='round' d='M2.047 5.19v13.62C2.047 21.124 6.504 23 12 23s9.953-1.875 9.953-4.19V5.19' />
		<path fill='gray' stroke='currentColor' strokeLinecap='round' strokeLinejoin='round' d='M12 9.381c5.498 0 9.954-1.875 9.954-4.19S17.498 1 12 1S2.048 2.875 2.048 5.19S6.504 9.382 12 9.382' />
		<path stroke='currentColor' strokeLinecap='round' strokeLinejoin='round' d='M21.953 9.38c0 2.316-4.457 4.191-9.953 4.191s-9.953-1.875-9.953-4.19m19.906 4.714c0 2.315-4.457 4.19-9.953 4.19s-9.953-1.875-9.953-4.19' />
	</g>
);

const IconGitLab = (
	<svg x='0' y='0' width='24' height='24' viewBox='0 0 32 32'>
...
const IconDocker = (
	<svg x='0' y='0' width='24' height='24' viewBox='0 0 256 185'>
...
const NODES: NodeConfig[] = [
	...
	{ x: 298, y: 100, icon: IconCloudflare, altIcon: IconKubernetes, swapOnHover: true },
	...
	return (
		...
		>
			<svg className='w-full h-full' viewBox='0 0 320 200' fill='none'>
				<defs>
					<filter id='int-shadow' x='-8%' y='-8%' width='116%' height='124%'>
						<feDropShadow dx='0' dy='1' stdDeviation='1.5' floodColor='black' floodOpacity='0.06' />
					</filter>

					{NODES.map((n, i) => (
						<path key={i} id={`int-p${i}`} d={getPath(n.x, n.y)} />
					))}
					...
					{NODES.map((_, i) => {
						...
						const dur = 3.4 + i * 0.2;
						return (
							<linearGradient key={`sg${i}`} id={`int-sig-${i}`} gradientUnits='userSpaceOnUse'>
								<stop offset='0%' stopColor='transparent' />
								<stop offset='15%' stopColor='var(--color-primary)' stopOpacity='0.06' />
								<stop offset='35%' stopColor='var(--color-primary)' stopOpacity='0.35' />
								<stop offset='50%' stopColor='var(--color-primary)' stopOpacity='0.8' />
								<stop offset='65%' stopColor='var(--color-primary)' stopOpacity='0.35' />
								<stop offset='85%' stopColor='var(--color-primary)' stopOpacity='0.06' />
				...
				{NODES.map((n, i) => {
					...
					return (
						<motion.path
							...
							transition={{
								pathLength: { duration: 0.6, delay: i * 0.08, ease: EASE_IN_OUT },
				...
				{phase >= 3 &&
					NODES.map((n, i) => (
						<path key={`sig${i}`} d={getPath(n.x, n.y)} stroke={`url(#int-sig-${i})`} strokeWidth='2' fill='none' />
					))}
				...
				>
					...
					<motion.circle
						...
						transition={isHovered ? { duration: 1.8, repeat: Infinity, ease: EASE_IN_OUT } : { duration: 0.3 }}
					/>
				...
				{NODES.map((n, i) => (
					...
					>
						<rect
							...
							stroke='var(--color-border)'
							strokeWidth='1'
							filter='url(#int-shadow)'
						/>
						<g transform={`translate(${n.x},${n.y}) scale(${ICON_SCALE}) translate(-12,-12)`}>
							{n.swapOnHover && n.altIcon ? (
								<AnimatePresence mode='popLayout' initial={false}>
									<motion.g
										key={isHovered ? `alt-${i}` : `pri-${i}`}
