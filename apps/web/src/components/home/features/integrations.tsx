...
/* Custom cubic-bezier curves (Emil: always use custom over built-in CSS) */
const EASE_OUT = [0.32, 0.72, 0, 1] as const;
const EASE_IN_OUT = [0.45, 0, 0.55, 1] as const;
/* Smooth deceleration for hover swap — fast entry, gentle settle */
const EASE_OUT_SMOOTH = [0.22, 0.68, 0, 1] as const;
...
/* ------------------------------------------------------------------ */
/*  SVG icons                                                          */
/*  Icons with non-24×24 viewBox use nested <svg> to normalize         */
/* ------------------------------------------------------------------ */
...
/* Vercel (256×222 → normalized to 24×24) */
const IconVercel = (
	<svg
		x='0'
		y='0'
...
/* Cloudflare (128×128 → normalized to 24×24) */
...
/* Slack (256×256 → normalized to 24×24) */
const IconSlack = (
	<svg
		x='0'
		y='0'
...
/* ---- Alt icons for hover swap ---- */

/* GitLab (32×32 → normalized to 24×24) */
const IconGitLab = (
	<svg
		x='0'
		y='0'
...
/* Docker (256×185 → normalized to 24×24) */
const IconDocker = (
	<svg
		x='0'
		y='0'
...
/* Kubernetes (128×128 → normalized to 24×24) */
...
/* ------------------------------------------------------------------ */
/*  Node config with primary and alternate icons                       */
/* ------------------------------------------------------------------ */
type NodeConfig = {
	x: number;
	y: number;
...
function getPath(nx: number, ny: number) {
	/* Horizontal nodes — straight line */
	if (Math.abs(ny - CY) < 15) return `M${nx},${ny} L${CX},${CY}`;
	/* Straight horizontal for ~60%, then diagonal into center */
...
/* Deterministic per-node direction: true = node→center, false = center→node */
const DIRECTIONS = [true, false, true, false, false, true];

/* ------------------------------------------------------------------ */
/*  Animation stages                                                   */
/*  Phase 0: only center hub visible                                   */
/*  Phase 1: nodes fade in with blur (staggered)                       */
/*  Phase 2: lines draw in                                             */
/*  Phase 3: shimmer dots active                                       */
/* ------------------------------------------------------------------ */
...
const IntegrationsIllustration = () => {
	...
	// Sequence phases when in view
	useEffect(() => {
		if (!isInView) return;

		// Phase 0 → 1: nodes fade in (after center settles)
		phaseTimers.current.push(setTimeout(() => setPhase(1), 500));
		// Phase 1 → 2: lines draw
		phaseTimers.current.push(setTimeout(() => setPhase(2), 1100));
		// Phase 2 → 3: shimmer dots
		...
		>
			...
			>
				<defs>
					...
					{/* Signal sweep gradients — one per path */}
					{NODES.map((_, i) => {
						...
						return (
							...
							>
								...
								<stop
									...
									stopColor='transparent'
								/>
								{/* Animate stop offsets to create sweep */}
								<animate
									attributeName='x1'
				...
				{/* ---- Connection lines (draw in at phase 2) ---- */}
				{NODES.map((n, i) => {
					...
					return (
						<motion.path
							...
							style={{
								strokeDashoffset: 0,
								// Override strokeDasharray during draw phase
				...
				{/* Dashed overlay (appears after draw completes) */}
				{phase >= 3 &&
					NODES.map((n, i) => (
						<motion.path
							key={`ld${i}`}
				...
				{/* ---- Signal sweeps along paths (phase 3) ---- */}
				{phase >= 3 &&
					NODES.map((n, i) => {
						const pathD = getPath(n.x, n.y);
						return (
							<g key={`sig${i}`}>
								{/* Signal gradient overlay on the path */}
								<path
									d={pathD}
				...
				{/* ---- Center hub (always visible, first to appear) ---- */}
				<motion.g
				...
				>
					...
					<motion.circle
						...
						transition={
							isHovered ? { duration: 1.8, repeat: Infinity, ease: EASE_IN_OUT } : { duration: 0.3 }
						}
					/>
					{/* Envval logo */}
				...
				{/* ---- Integration nodes with hover swap ---- */}
				{NODES.map((n, i) => (
					<motion.g
						key={`n${i}`}
					...
					>
						...
						{/* Icon — only swap if swapOnHover, otherwise static */}
