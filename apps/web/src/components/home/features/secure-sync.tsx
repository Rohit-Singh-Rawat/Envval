		...
		const frame = () => {
			...
			for (let i = 0; i < len; i++) {
				...
				if (charProgress >= 1) {
					...
					chars.push(CIPHER_CHARS[Math.floor(Math.random() * CIPHER_CHARS.length)]);
				} else {
					chars.push(text[i] ?? '');
				}
			}
...
async function runSyncSequence(
	...
	const gradRight = q<SVGLinearGradientElement>('[data-signal-gradient="right"]');
	...
	rightVal0.className = 'text-[11px] font-mono leading-[22px] text-muted-foreground/60 truncate';
	...
	leftVal1.className = 'text-[11px] font-mono leading-[22px] text-muted-foreground/60 truncate';
	...
	if (cancelled()) return;
	await Promise.all([
		animate('[data-right-line-0]', { height: 0 }, { duration: 0.35, ease: EASE_OUT }),
		animate('[data-right-line-1]', { height: 0 }, { duration: 0.35, ease: EASE_OUT }),
	]);
	if (cancelled()) return;
	await sleep(200);
	...
	if (cancelled()) return;
	await Promise.all([
		animate('[data-left-line-0]', { height: 0 }, { duration: 0.35, ease: EASE_OUT }),
		animate('[data-left-line-1]', { height: 0 }, { duration: 0.35, ease: EASE_OUT }),
	]);
	if (cancelled()) return;
