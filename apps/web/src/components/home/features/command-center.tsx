...
const REDIS_VAR: EnvVariable = {
  key: "REDIS_URL",
  val: "redis://cache:6379",
  visible: true,
};
...
function EnvVariableRow({ variable }: { variable: EnvVariable }) {
  return (
    <div className="flex items-center py-[5px] px-3 font-mono">
      ...
      <span
        className={`text-[9px] truncate ${variable.visible ? "text-foreground/55" : "text-muted-foreground/35"}`}
      >
        {variable.val}
      </span>
    </div>
...
async function runSequence(
  ...
  setDeviceOverrides: React.Dispatch<
  ...
  while (!cancelled()) {
    ...
    setLocalVars((prev) =>
      prev.filter((v) => v.key !== "STRIPE_KEY" && v.key !== "REDIS_URL"),
    );
    await sleep(1500);
    if (cancelled()) return;
