...
export const keyMaterialApi = honoFactory
  ...
  .post(
    "/",
    zValidator("json", keyMaterialSchema),
    authMiddleware,
    async (c) => {
      ...
      try {
        const wrappedUserKey =
          await keyMaterialService.getWrappedKeyMaterialForSession(
            session.id,
            publicKey,
          );
        ...
        const message =
          error instanceof Error ? error.message : "Failed to get key material";
