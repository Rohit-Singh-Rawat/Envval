...
export const extensionApi = new Hono()
  ...
  .post(
    ...
    async (c) => {
      ...
      logger.info("Extension device token request received", {
      ...
      if (!isExtension) {
        logger.warn(
          "Non-extension client attempted to use extension endpoint",
          { userAgent },
        );
        return c.json(
          {
      ...
      try {
        ...
        logger.info("Device authorization successful, access token received", {
          accessToken,
        });
          ...
          return c.json(
            {
              error: "invalid_grant",
              error_description:
                "Failed to get session after device authorization",
            },
            400,
          );
        }
        ...
        const deviceRecord = await deviceService.ensureExists(
          deviceId,
          userId,
          "DEVICE_EXTENSION",
          {
        ...
        const wrappedKeyMaterial = wrapKeyMaterialForDevice(
          body.public_key,
          keyMaterial,
        );
        ...
        logger.info("Session updated with extension data", {
          sessionId,
          deviceId,
        });
        ...
        logger.info("Extension device token response sent successfully", {
          deviceId,
        });
   ...
   */
  .post(
    "/device/refresh-session",
    rateLimitMiddleware({ tier: "mutation", by: "ip" }),
    async (c) => {
      ...
      const [sessionRecord] = await db
        ...
        .where(
          and(
            ...
            eq(sessionTable.sessionType, "SESSION_EXTENSION"),
          ),
        )
        .limit(1);
      ...
      if (!sessionRecord) {
        logger.info(
          "Extension session refresh rejected: session not found or revoked",
        );
        return c.json(
          {
            error: "session_revoked",
            error_description: "Session has been revoked or does not exist",
          },
          401,
        );
      }
      ...
      if (Date.now() - sessionRecord.createdAt.getTime() > MAX_SESSION_AGE_MS) {
        logger.warn(
          "Extension session refresh rejected: session max age exceeded",
          {
            sessionId: sessionRecord.id,
            ageDays: Math.round(
              (Date.now() - sessionRecord.createdAt.getTime()) /
                (24 * 60 * 60 * 1000),
            ),
          },
        );
      ...
      const newExpiresAt = new Date(
        Date.now() + SESSION_RENEWAL_DAYS * 24 * 60 * 60 * 1000,
      );
