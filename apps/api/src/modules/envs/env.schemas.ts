...
const envFileNameSchema = z
  ...
  .max(
    MAX_ENV_FILE_NAME_LENGTH,
    `File name must be ${MAX_ENV_FILE_NAME_LENGTH} characters or less`,
  )
  .regex(
    ENV_FILE_NAME_PATTERN,
    "File name must be a valid env file path (e.g. .env, apps/api/.env.local)",
  )
...
export const createEnvSchema = z.object({
  ...
  content: z
    ...
    .max(
      MAX_ENV_CONTENT_LENGTH,
      `Content exceeds ${MAX_ENV_CONTENT_LENGTH / 1000}KB limit`,
    ),
  ...
  envCount: z
    ...
    .int()
    .nonnegative()
    .max(MAX_ENV_VAR_COUNT, `Variable count exceeds ${MAX_ENV_VAR_COUNT}`)
    .default(0),
});
