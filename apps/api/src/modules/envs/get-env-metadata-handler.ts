...
export const getEnvMetadataHandler = honoFactory.createHandlers(
	...
	async (ctx) => {
		...
		try {
			...
			const result = await envService.getEnvironmentMetadataById(user.id, envId);
			...
			logger.error('Failed to get environment metadata', { error: error instanceof Error ? error.message : String(error) });
			return ctx.json(
				{ error: 'Failed to retrieve environment metadata' },
