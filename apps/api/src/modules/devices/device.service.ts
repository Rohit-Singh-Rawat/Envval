...
export class DeviceService {
	...
	static async deleteDevice(deviceId: string) {
		return await db.transaction(async (tx) => {
			...
			const [deleted] = await tx.delete(device).where(eq(device.id, deviceId)).returning();
	...
	static async deleteAllDevicesExcept(userId: string, exceptDeviceId: string) {
		return await db.transaction(async (tx) => {
			...
			// Delete the devices
			const deleted = await tx.delete(device).where(inArray(device.id, deviceIds)).returning();
