...
export class RepoService {
  ...
  async getEnvironments(
    ...
    if (includeContent) {
      const environments = await db
        ...
        .from(environment)
        .where(
          and(eq(environment.repoId, repoId), eq(environment.userId, userId)),
        );
      return environments;
    }
  ...
  async updateRepository(
    ...
    if (updateData.slug) {
      const existingWithSlug = await this.getRepositoryBySlug(
        userId,
        updateData.slug,
      );
  ...
  async migrateRepository(
    ...
    return await db.transaction(async (tx) => {
      ...
      const [conflictRepo] = await tx
        .select()
        .from(repo)
        .where(eq(repo.id, newRepoId));
      if (conflictRepo) {
      ...
      const environments = await tx
        ...
        .where(
          and(
            eq(environment.repoId, oldRepoId),
      ...
      if (newEnvRecords.length > 0) {
        ...
        for (const newRec of newEnvRecords) {
          ...
          const oldEnv = environments.find(
            (e) => e.fileName === newRec.fileName,
          );
          if (oldEnv) {
            await tx
              .update(auditLog)
