...
export const repoIdHashSchema = z
  ...
  .regex(
    /^[a-f0-9]{64}$/,
    "Invalid repository ID format: expected SHA-256 hash",
  );

/**
 * UUID format for legacy repository identification.
 */
export const repoIdUuidSchema = z
  .string()
  .uuid("Invalid repository ID format: expected UUID");
...
export const repoCreateBodySchema = z.object({
  ...
  workspacePath: z
    .string()
    .min(1, "Workspace path is required")
