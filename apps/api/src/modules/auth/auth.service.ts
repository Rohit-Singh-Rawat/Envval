          ...
          try {
            ...
            emailService
              .sendNewDeviceLoginEmail(user.email, {
                ...
                signInType: resolveSignInLabel(
                  latestAccount?.providerId,
                  sessionType,
                ),
            ...
            logger.error("Error in session create hook", {
    ...
    user: {
      create: {
        before: async (user) => {
          ...
          const { ciphertext, iv, keyId } =
            encryptKeyMaterialWithMaster(keyMaterial);
          return {
            data: {
              ...user,
...
function sanitizeUser<T extends Record<string, unknown>>(
  ...
  const { keyMaterialEnc, keyMaterialIv, keyMaterialKeyId, ...rest } =
    user as T & {
  ...
  return rest as Omit<
    T,
    "keyMaterialEnc" | "keyMaterialIv" | "keyMaterialKeyId"
  >;
}
...
function resolveSignInLabel(
  ...
  return sessionType === "SESSION_EXTENSION"
