...
export class KeyMaterialService {
	 ...
	 */
	async getWrappedKeyMaterialForSession(sessionId: string, publicKey: string): Promise<string> {
		...
		if (sessionRecord.keyMaterialDelivered) {
			throw new Error('Key material has already been delivered for this session');
		}
