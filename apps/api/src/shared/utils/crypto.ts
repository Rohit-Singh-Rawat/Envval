 ...
 */
export function wrapKeyMaterialForDevice(
  publicKeyPem: string,
  keyMaterial: string,
): string {
 ...
 */
export function unwrapKeyMaterialForDevice(
  privateKeyPem: string,
  wrappedB64: string,
): string {
...
export function encryptKeyMaterialWithMaster(keyMaterial: string) {
  ...
  const ciphertext = Buffer.concat([
    cipher.update(keyMaterial, "utf8"),
    cipher.final(),
  ]);
 ...
 */
export function decryptKeyMaterialWithMaster(
