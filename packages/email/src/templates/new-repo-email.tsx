...
const NewRepoEmailTemplate = (props: NewRepoEmailData): React.ReactElement => {
  ...
  return (
    <EmailLayout productName={productName} logoUrl={logoUrl}>
      ...
      >
        A new repository{" "}
        <strong style={{ color: REPO_NAME_COLOR }}>{repoName}</strong> has been
        successfully added to your <strong>{productName}</strong> account. You
        can now manage environment variables for this project from your
        dashboard or directly in your editor.
      </p>

      {repoUrl && <PrimaryButton href={repoUrl} label="View Repository" />}
